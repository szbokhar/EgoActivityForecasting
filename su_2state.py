...
def hc_only_make_sarsa_lists(rl_config):
    ...
    rl_config.q_shape = [rl_config.voxel_grid.shape[0], rl_config.voxel_grid.shape[2], 4, len(rid2rl_actions.keys())]
...
def hc_only_reset(rl_config):
    ...
    e_state[rl_state2id['Phase']] = randint(3)
...
def hc_only_reward(rl_config, state, action, new_state):
    ...
    if rl_config.hc_pos is None:
        ...
        for i in range(sarsa.shape[0]):
            if rid2rl_actions[sarsa[i,state_size]] == "Do_MakeHC":
    ...
    if rl_config.wash_pos is None:
        wash_data = np.zeros((0,2))
        for i in range(sarsa.shape[0]):
            if rid2rl_actions[sarsa[i,state_size]] == "Do_WashCup":
                wash_data = np.concatenate((wash_data, sarsa[i, [rl2id['Pos_X'],rl2id['Pos_Y']]].reshape((1,2))), axis=0)

        rl_config.wash_pos = scipy.spatial.KDTree(wash_data)


    hcpos = rl_config.hc_pos
    washpos = rl_config.wash_pos
    ...
    if action == rl_actions2rid['Do_WashCup']:
        (adist,_) = washpos.query(state[posidx])
        reward += -adist*actionP
...
def hc_only_transition(rl_config, state, act, Q):
    ...
    if rid2rl_actions[act] == 'Move_North':
        ...
        if state[rl_state2id['Phase']] == 2:
            next_state[rl_state2id['Phase']] = 3
            isFinished = True
        else:
            act = -1
